// Patched version 0.04. Now with show, hide and toggle functions. (Andreas Haller)
/*
...
InkWeb.show = function (el) {
  this.setStyle(el, "display", "inline");
}

InkWeb.toggle = function (el) {  
  if(this.isHidden(el)) {
    this.show(el);
  } else {
    this.hide(el);
...
InkWeb.isHidden = function (el) {
  var display = this.getStyle(el, "display");
  // If display style is not defined, the element is not hidden
